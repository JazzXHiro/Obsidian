...
**Lanes:** Customer, System, Payment Gateway  
Steps distributed across lanes clearly.
...
sequenceDiagram
    ...
    User->>GUI: Enter dates, room type, guests
    GUI->>Controller: searchAvailability()
    Controller->>Availability: checkRange()
    Availability-->>Controller: available/not

    alt Available
        Controller->>GUI: showRooms()
        User->>GUI: selectRoom + details
        GUI->>Controller: confirmBooking()
        Controller->>Availability: validateConstraints()
        Availability-->>Controller: OK
        Controller->>Payment: makePayment()
        Payment-->>Controller: success
        Controller->>DB: saveReservation()
        DB-->>Controller: reservationID
        Controller->>GUI: showConfirmation()
    else Not available
        Controller->>GUI: showError()
    end
```
...
- Class diagram = blueprint; shows classes.
    
- Object diagram = snapshot; shows objects at a moment.
...
If you want **PDF-ready notes**, **diagram PNGs**, or **hand-drawn style diagrams**, tell me!
