...
**(Write this version in exam; simple & correct)**
...
- Clients access LMS via browser
    
- Web server hosts UI
    
- Application server runs logic (book issue, return, search)
    
- Database server stores books, members, transactions
...
**Lifelines:** User → App → Police → Verifier → Agency → Court

```
User -> App: uploadEvidence()
App -> Police: notify()
Police -> Verifier: sendEvidence()
Verifier -> Agency: verifyStatus()
alt verified
    Agency -> User: sendMail/SMS()
    Agency -> Court: submitEvidence()
    Court -> Agency: finalJudgement()
else rejected
    Verifier -> Police: invalidEvidence()
end
```
...
**Swimlane Actors:** Student, Codetantra System
...
If you want, I can generate **all diagrams as clean PNGs** for printing.
